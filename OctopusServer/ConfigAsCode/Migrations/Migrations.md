...
When we consider that the state transition for an OCL file is potentially dependent on the state of the database, the complexity arises when we realise that the state of the database changes _independently_ of the state of the OCL files.

![Independant Migrations](./Independant_migrations.png)

### Repeatable Migrations
If the transition of an OCL file is potentially a two way interaction with a database, how should we handle the migration having to take place potentially multiple times, across branches?
...
- Minimise the dependency between transformations with OCL and database cross context
  - Avoid destrictuve changes from the database that are non-rerunnable
...
## Execution
Database migrations executed when the program starts.
This won't work in the world of git migrations for a number of reasons:
...
Instead of executing migrations on application startup, git migrations are executed at runtime when loading the OCL from the git repository.
The migrations are applied without making any commits, the changes are only committed alongside any other changes made by the user through Octopus Server.
...
Today, a handfull of project entities can be stored in git as part of the Config as Code feature. As we allow for more entities to be stored in git, we need to allow for execting Config as Code enabled projecs to import documents from the database.
...
To address the first issue, when an `Import` step is executed, any proceeding steps for that same document are skipped:
...
This is only necessary if we need to import and remove something a document in one "session" (a set of migrations, E.g: v3 👉 v5)
